...
class AFKCommand(SimpleCommandPlugin):
        ...
        try:
            ...
            if self.protocol is not None:
                self.protocol.send_chat_message(
                    'Reload failed! Please check config.json!'
                )
                self.protocol.send_chat_message(
    ...
    def afk(self, data):
        ...
        if self.protocol.player.name in self.afk_list:
            if self.afk_list[self.protocol.player.name] is True:
    ...
    def set_afk_status(self, name):
        ...
        if self.config.chattimestamps:
            timestamp = '^gray;<{}> '.format(now.strftime('%H:%M'))
        else:
            timestamp = ''
        if name in self.afk_list:
            if self.afk_list[name] is False:
                self.factory.broadcast(
                    '{}{} ^gray;{}'.format(
                        timestamp,
                        self.player_manager.get_by_name(name).colored_name(
                            self.config.colors
                        ),
                        self.afk_message
                    )
