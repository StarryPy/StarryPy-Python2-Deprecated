...
class PlanetVisitorAnnouncer(BasePlugin):
    ...
    def after_player_warp(self, data):
        ...
        if (
                w.warp_action['warp_type'] == 1 or
                (
                    w.warp_action['warp_type'] == 3 and
                    w.warp_action['warp_action_type'] == 1
                )
        ):
    ...
    def announce_on_planet(self, who_beamed):
        self.factory.broadcast_planet(
            '{}^green; beamed down to the planet'.format(
                who_beamed.colored_name(self.config.colors)
            ),
