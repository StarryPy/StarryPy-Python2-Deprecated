...
class PartyChatPlugin(SimpleCommandPlugin):
    ...
    def party(self, data):
        ...
        if self.config.chattimestamps:
            timestamp = '^#44aadd;<{}> ^yellow;'.format(now.strftime('%H:%M'))
        else:
            timestamp = ''
        if not data:
        ...
        try:
            ...
            if not senders_team:
                self.protocol.send_chat_message(
                    '^red;You\'re not part of a team.^yellow;'
                )
                return
            ...
            for protocol in self.factory.protocols.itervalues():
                self.logger.vdebug(
                    '%s, %s',
                    protocol.player.name, protocol.player.party_id
                )
                if protocol.player.party_id == senders_team:
                    protocol.send_chat_message(
                        '{}{}Party: ^yellow;<{}^yellow;> {}{}'.format(
                            timestamp,
                            '^#44aadd;',
    ...
    def party_check(self, data):
        try:
            ...
            self.logger.debug(
                '%s checked for team when variable was not set.',
    ...
    def teamInvite(self, leader, invitee, party_id):
        self.logger.vdebug(
            '%s invited %s to a team (id: %s)', leader, invitee, party_id
        )
        self.protocol.player.party_id = party_id

    def teamJoin(self, leader, invitee, party_id):
        self.logger.vdebug(
            '%s joined %s\'s team (id: %s)', invitee, leader, party_id
        )
        self.protocol.player.party_id = party_id
