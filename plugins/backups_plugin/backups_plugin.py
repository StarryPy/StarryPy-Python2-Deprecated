...
class BackupsPlugin(SimpleCommandPlugin):
    ...
    def activate(self):
        ...
        self.player_manager = (
            self.plugins['player_manager_plugin'].player_manager
        )
        self.db = DatabaseManager(os.path.join('backups', 'backups.db'))
    ...
    def backup_status(self, data):
        ...
        if not data:
            ...
            if on_ship:
                self.protocol.send_chat_message(
                    'You need to either be on a planet or provide '
                    'a target player and planet.'
                )
                self.protocol.send_chat_message(usage)
                return
            ...
            sql = (
                'SELECT active, backup_logs, planet_name FROM '
                'backups WHERE planet_coord = ?'
            )
            result = self.db.select(sql, (current_planet, ))
            if not result:
                self.protocol.send_chat_message(
                    'Planet not currently being backed up.'
                )
                return
        ...
        if data:
            ...
            if not planet_name:
                self.protocol.send_chat_message(
                    'A planet name must be provided.'
                )
                self.protocol.send_chat_message(usage)
                return
            ...
            sql = (
                'SELECT active, backup_logs FROM backups '
    ...
    def backup_enable(self, data):
        ...
        if active:
            self.protocol.send_chat_message(
                'Planet backups are already enabled.'
            )
            return
    ...
    def backup_manual(self, data):
        ...
        if not data:
            ...
            sql = (
                'SELECT backup_logs, planet_name, owner FROM backups '
                'WHERE planet_coord = ?'
            )
            result = self.db.select(sql, (current_planet, ))
            if not result:
        ...
        if data:
            ...
            sql = (
                'SELECT backup_logs, planet_coord FROM backups '
                'WHERE owner = ? AND planet_name = ?'
            )
            result = self.db.select(sql, (player_name, planet_name))
            if not result:
        ...
        planet_file = '{}.world'.format('_'.join(current_planet.split(':')))
        ...
        if timestamp in times:
            self.protocol.send_chat_message(
                'Planet has been backed up within the last minute.'
            )
            self.logger.debug(
                'Aborting manual backup - recent backup already present.'
            )
            return

        times.append(timestamp)

        src = os.path.join(self.config.starbound_path, 'universe', planet_file)
        ...
        self.logger.info(
            'Backup command succeeded: New backup added for planet %s.',
            planet_name
        )
    ...
    def backup_add(self, data):
        """
        Add current planet into the backups system with the given player as
        its owner.
        Syntax: /backup add (player) (planet name)
        """
        usage = (
            'Add current planet into the backups system with the given '
            'player as its owner.\nSyntax: /backup add (player) (planet name)'
        )
        ...
        if self.db.select(sql, (current_planet, )):
            self.protocol.send_chat_message(
                'Planet is already being backed up.'
            )
            return
        ...
        self.logger.info(
            '%s added planet %s into backups for %s as name %s.',
        ...
        sql = (
            'INSERT INTO backups (planet_coord, planet_name, owner, '
            'active, backup_logs) VALUES (?,?,?,?,?)'
        )
        arg = (
            current_planet, planet_name, player_name, True, json.dumps(times)
        )
        self.db.insert(sql, arg)

        self.protocol.send_chat_message(
            'Success! Planet is now being backed up.'
        )
        self.logger.info(
            'Backup command succeeded: Planet now being backed up.'
        )
    ...
    def backup_restore(self, data):
        ...
        if not timestamp:
            self.protocol.send_chat_message(
                'A timestamp restore point must be provided.'
            )
            self.protocol.send_chat_message(usage)
            return
        ...
        if not self._validate_timestamp(timestamp):
            self.protocol.send_chat_message(
                'Timestamp provided was not in a valid format.'
            )
        ...
        if timestamp not in logs:
            self.protocol.send_chat_message(
                'Timestamp provided is not a valid restore point. '
                'Use `/backup status` to find a restore point.'
            )
            return

        who = [
            w.name for w in self.player_manager.who()
            if self._name_scrape(w.planet) == current_planet and not w.on_ship
        ]
        self.logger.vdebug('Players still on planet: %s.', who)
        if who:
            self.protocol.send_chat_message(
                'Error: Cannot restore backup while people'
                ' are still on the planet.'
            )
            return

        planet_file = '{}.world'.format('_'.join(current_planet.split(':')))
        src = os.path.join('backups', player_name, planet_name)
        dst_path = os.path.join('.', 'backups', player_name, planet_name)
        dst = os.path.join(dst_path, '{}_{}'.format(planet_file, timestamp))
        self._copy_file(src, dst)

        self.protocol.send_chat_message(
            'The planet has been successfully restored.'
        )
        self.logger.info(
            '%s restored planet %s from backups from backup %s.',
            self.protocol.player.name, planet_name, timestamp
        )
    ...
    def _validate_timestamp(self, timestamp):
        """
        Validate a user-provided timestamp, to be sure it is the
         correct formatting.
        """
        try:
