...
class PluginManagerPlugin(SimpleCommandPlugin):
    ...
    def plugin_list(self, data):
        ...
        inactive_plugins = [
            ...
            if plugin not in self.plugin_manager.plugins.iterkeys()
        ]
    ...
    def plugin_disable(self, data):
        ...
        try:
            ...
            self.protocol.send_chat_message(
                'Couldn\'t find a plugin with the name %s'.format(data[0])
            )
            return

        if plugin is self:
            self.protocol.send_chat_message(
                'Sorry, this plugin can\'t be deactivated.'
            )
            return
        if not plugin.active:
            self.protocol.send_chat_message(
                'That plugin is already deactivated.'
            )
            return
    ...
    def plugin_enable(self, data):
        ...
        try:
            if self.plugin_manager.plugins[data[0]].active:
                self.protocol.send_chat_message(
                    'That plugin is already active.'
                )
                return
        except KeyError:
            # plugin isn't in loaded plugins, so try and load it
            try:
    ...
    def help(self, data):
        ...
        if data:
            ...
            if func is None:
                self.protocol.send_chat_message(
                    'Couldn\'t find a command with the name ^yellow;%s'.format(
                        command
                    )
                )
                ...
                self.protocol.send_chat_message('{}'.format(func.__doc__))
        else:
            available = []
            ...
            self.protocol.send_chat_message(
                'Available commands: ^yellow;{}\n^green;Get more help on '
                'commands with ^yellow;/help [command]'.format(
                    '^green;, ^yellow;'.join(available)
                )
            )
            return True
