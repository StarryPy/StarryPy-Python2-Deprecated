...
class StarryPyIrcBot(irc.IRCClient):
    ...
    def connectionLost(self, reason):
        ...
        self.logger.info("IRC connection lost: %s", format(reason))
    ...
    def joined(self, target):
        ...
        self.logger.debug("Sucesssfully joined the IRC channel %s.", self.factory.target)
...
class StarryPyIrcBotFactory(protocol.ReconnectingClientFactory):
    ...
    def clientConnectionFailed(self, connector, reason):
        self.logger.error("IRC connection failed: %s" % format(reason))
