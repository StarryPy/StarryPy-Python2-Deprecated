...
class IrcPlugin(BasePlugin):
    ...
    def activate(self):
        ...
        if not getattr(self, 'irc_factory', None):
            self.irc_factory = StarryPyIrcBotFactory(self.channel, self.logger, self.nickname, self.nickserv_password, self.factory, self.config.colors, self.echo_from_channel)
