...
class EmotesPlugin(SimpleCommandPlugin):
    ...
    def me(self, data):
        ...
        Syntax: /me (emote)
        Predefined emotes: ^yellow;beckon^green;, ^yellow;bow^green;,
        ^yellow;cheer^green;, ^yellow;cower^green;, ^yellow;cry^green;,
        ^yellow;dance^green;, ^yellow;hug^green;, ^yellow;hugs^green;,
        ^yellow;kiss^green;, ^yellow;kneel^green;, ^yellow;laugh^green;,
        ^yellow;lol^green;, ^yellow;no^green;, ^yellow;point^green;,
        ^yellow;ponder^green;, ^yellow;rofl^green;, ^yellow;salute^green;,
        ^yellow;shrug^green;, ^yellow;sit^green;, ^yellow;sleep^green;,
        ^yellow;surprised^green;, ^yellow;threaten^green;, ^yellow;wave^green;,
        ^yellow;yes^green;
        ...
        if self.protocol.player.muted:
            self.protocol.send_chat_message(
                'You are currently muted and cannot emote. '
                'You are limited to commands and admin chat ('
                'prefix your lines with {} for admin chat.'.format(
        ...
        if emote == 'beckon':
            ...
            spec_prefix = '^cyan;!'
            emote = 'flips a coin and its... ^cyan;{}'.format(choice(flipdata))
        elif emote == 'hug':
            emote = 'needs a hug!'
        elif emote == 'hugs':
            ...
            spec_prefix = '^cyan;!'
            emote = 'rolled ^cyan;{}'.format(randrange(1, 101))
        elif emote == 'salute':
            emote = 'salutes you'
        elif emote == 'shrug':
            emote = 'shrugs at you'
        ...
        if self.config.chattimestamps:
            timestamp = '^orange;<{}> '.format(now.strftime('%H:%M'))
        else:
            timestamp = ''
        self.factory.broadcast_planet(
            '{}{}{}'.format(
                timestamp,
                spec_prefix,
                '^orange;{} {}'.format(self.protocol.player.name, emote)
            ),
